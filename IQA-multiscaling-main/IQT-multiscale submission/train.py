...
config = Config({
    ...
    "GPU_ID": "0",
    "num_workers": 8,
    "scale":1, #could be changed as per requirement available options 0.5,1,2,3 where 0.5 is upscale by 2 and 2 and 3 are downscale by 2 and 3 factor
    ...
    "db_path": "/home/ubuntu/Documents/datasets/PIPAL/Train_Original_Images",         # root of dataset
    "snap_path": "./weights/PIPAL",                                         # path for saving weights
    "txt_file_name": "/home/ubuntu/PycharmProjects/our baseline with 1.58 score/IQT2/IQA_list/PIPAL.txt",                    # image list file (.txt)
    "train_size": 0.8,
    "scenes": "all",
...
if config.scale != 1:
    train_dataset = IQADataset(
        ...
        transform=transforms.Compose([Normalize(0.5, 0.5), RandHorizontalFlip(), RandRotation(), ToTensor()]),
        train_mode=True,
    ...
    train_dataset = IQADataset(
        ...
        transform=transforms.Compose(
            [RandCrop(config.crop_size), Normalize(0.5, 0.5), RandHorizontalFlip(), RandRotation(), ToTensor()]),
        train_mode=True,
        scene_list=train_scene_list,
        train_size=config.train_size
    )
