...
def train_epoch(config, epoch, model_transformer, model_backbone, save_output, criterion, optimizer, scheduler, train_loader):
    ...
    for data in tqdm(train_loader):
        ...
        feat_ref, feat_diff = model_backbone(r_img, d_img, save_output)
        if config.scale != 1:
            feat_ref = torch.nn.functional.interpolate(feat_ref, size=(21, 21), mode='bilinear', align_corners=True)
            feat_diff = torch.nn.functional.interpolate(feat_diff, size=(21, 21), mode='bilinear', align_corners=True)
        ...
        for data in tqdm(test_loader):
            ...
            if config.test_ensemble:    # use test ensemble
                ...
                for i in range(config.n_ensemble):
                    ...
                    if config.scale != 1:
                        feat_ref = torch.nn.functional.interpolate(feat_ref, size=(21, 21), mode='bilinear',
                                                                   align_corners=True)
                        feat_diff = torch.nn.functional.interpolate(feat_diff, size=(21, 21), mode='bilinear',
                                                                    align_corners=True)
                    # backbone feature map (ref)
