...
class InceptionResNetV2(nn.Module):
    ...
    def features(self, input):  # input = (bacth, 3, 256, 256)
        x = self.conv2d_1a(input)   # x = (batch, 32, 127, 127) after the line executes
        x = self.conv2d_2a(x)       # x = (batch, 32, 125, 125)
        x = self.conv2d_2b(x)       # x = (batch, 64, 125, 125)
        x = self.maxpool_3a(x)      # x = (batch, 64, 62, 62)
        x = self.conv2d_3b(x)       # x = (batch, 80, 62, 62)
        x = self.conv2d_4a(x)       # x = (batch, 192, 60, 60)
        x = self.maxpool_5a(x)      # x = (batch, 192, 29, 29)
        
        x = self.mixed_5b(x)        # x = (batch, 320, 29, 29)
        x = self.repeat(x)          # x = (batch, 320, 29, 29)
    ...
    def forward(self, input_ref, input_dis, save_output):
        ...
        return feat_ref, feat_ref - feat_dis
